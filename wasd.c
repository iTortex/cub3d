...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/29 15:20:47 by amarcele          #+#    #+#             */
/*   Updated: 2020/09/29 15:21:55 by amarcele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	w_move(t_file *file, double ms)
{
	...
	if (file->map[(int)(file->game.posx + file->game.dirx * ms)]
	[(int)(file->game.posy)] != '1')
		file->game.posx += file->game.dirx * ms;
	if (file->map[(int)(file->game.posx)][(int)(file->game.posy +
	file->game.diry * ms)] != '1')
...
{
	mlx_clear_window(file->img.mlx, file->img.mlx_win);
	if (file->map[(int)(file->game.posx - file->game.diry * ms)]
	[(int)(file->game.posy)] != '1')
