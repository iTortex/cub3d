...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/29 13:50:19 by amarcele          #+#    #+#             */
/*   Updated: 2020/09/29 16:00:16 by amarcele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void		first_count(t_file *file)
{
	file->game.camerax = 2 * (double)file->ioooo /
	(double)file->win.width - 1;
	file->game.raydirx = file->game.dirx + file->game.planex
	* file->game.camerax;
	file->game.raydiry = file->game.diry + file->game.planey
	* file->game.camerax;
...
static void		nextstep(t_file *file)
{
	...
	{
		file->game.stepy = -1;
		file->game.sidedisty = (file->game.posy - file->game.mapy)
		* file->game.deltadisty;
	}
	else
	{
		file->game.stepy = 1;
		file->game.sidedisty = (file->game.mapy + 1.0 - file->game.posy)
		* file->game.deltadisty;
	}
...
static void		firstwhile(t_file *file)
{
	while (file->ioooo < file->win.width)
	{
		...
		{
			file->game.stepx = -1;
			file->game.sidedistx = (file->game.posx - file->game.mapx)
			* file->game.deltadistx;
		}
		else
		{
			file->game.stepx = 1;
			file->game.sidedistx = (file->game.mapx + 1.0 - file->game.posx)
			* file->game.deltadistx;
		}
		nextstep(file);
		file->ioooo++;
